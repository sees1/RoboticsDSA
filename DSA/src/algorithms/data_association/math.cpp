...
namespace math {
  double normalCDF(double u)
  {
    ...
    static const double c[9] = {2.15311535474403846e-8, 5.64188496988670089e-1, 8.88314979438837594e00,
                                6.61191906371416295e01, 2.98635138197400131e02, 8.81952221241769090e02,
                                1.71204761263407058e03, 2.05107837782607147e03, 1.23033935479799725E03};
    static const double d[9] = {1.00000000000000000e00, 1.57449261107098347e01, 1.17693950891312499e02,
    ...
    if (std::isnan(u))
      throw std::runtime_error("normalCDF: u is nan!");
    if (!std::isfinite(u))
      throw std::runtime_error("normalCDF: u is non finite!");
  ...
  double normalQuantile(double p)
  {
    ...
    if (std::isnan(p))
      throw std::runtime_error("normalQuantile: p is nan!");
    if (p >= 1.0 || p <= 0.0)
      throw std::runtime_error("normalQuantile: p is out of bound! 0 < p < 1");
  ...
  {
    if (p < 0 || p >= 1)
      throw std::out_of_range("chi2inv: p shoud be in range 0 <= p < 1");

    if (std::abs(p - 0.0) < 1e-10)
      return 0;
    else
